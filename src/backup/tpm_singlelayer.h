...
void tpm_makeinput(struct ivec v)
{
...
void tpm_makewt(struct ivec v, int L, int U)
{
      int mod = U - L + 1;
      for(int i=0; i<v.size; ++i){
...
void tpm_parity(struct tpm* tpm, struct ivec* inpvecs)
{
      int parity = 1;
...
void tpm_learn(struct tpm* tpm, struct ivec* inpvecs)
{
      for(int i=0; i<tpm->K; ++i){
...
int gsynchronized(struct tpm* tpms, int gsize)
{
...
int synchronized(struct tpm* tpmA, struct tpm* tpmB)
{
